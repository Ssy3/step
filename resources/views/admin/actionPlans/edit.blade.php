...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.actionPlan.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.action-plans.update", [$actionPlan->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="title">{{ trans('cruds.actionPlan.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $actionPlan->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('updates') ? 'is-invalid' : '' }}" name="updates" id="updates">{!! old('updates', $actionPlan->updates) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('initiative') ? 'is-invalid' : '' }}" name="initiative_id" id="initiative_id" required>
                    @foreach($initiatives as $id => $initiative)
                        <option value="{{ $id }}" {{ (old('initiative_id') ? old('initiative_id') : $actionPlan->initiative->id ?? '') == $id ? 'selected' : '' }}>{{ $initiative }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('risks') ? 'is-invalid' : '' }}" type="text" name="risks" id="risks" value="{{ old('risks', $actionPlan->risks) }}">
                @if($errors->has('risks'))
                    <div class="invalid-feedback">
                        {{ $errors->first('risks') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('resources') ? 'is-invalid' : '' }}" type="text" name="resources" id="resources" value="{{ old('resources', $actionPlan->resources) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('start') ? 'is-invalid' : '' }}" type="text" name="start" id="start" value="{{ old('start', $actionPlan->start) }}">
                @if($errors->has('start'))
                    <div class="invalid-feedback">
                        {{ $errors->first('start') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('end') ? 'is-invalid' : '' }}" type="text" name="end" id="end" value="{{ old('end', $actionPlan->end) }}">
                @if($errors->has('end'))
                    <div class="invalid-feedback">
                        {{ $errors->first('end') }}
                    </div>
            ...
            <div class="form-group">
                <label>{{ trans('cruds.actionPlan.fields.approval') }}</label>
                <select class="form-control {{ $errors->has('approval') ? 'is-invalid' : '' }}" name="approval" id="approval">
                    <option value disabled {{ old('approval', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\ActionPlan::APPROVAL_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('approval', $actionPlan->approval) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.actionPlan.fields.approval_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('users') ? 'is-invalid' : '' }}" name="users[]" id="users" multiple>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (in_array($id, old('users', [])) || $actionPlan->users->contains($id)) ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('users'))
                    <div class="invalid-feedback">
