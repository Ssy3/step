    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.risks.update", [$risk->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $risk->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('probability') ? 'is-invalid' : '' }}" type="number" name="probability" id="probability" value="{{ old('probability', $risk->probability) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('impact') ? 'is-invalid' : '' }}" type="number" name="impact" id="impact" value="{{ old('impact', $risk->impact) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('gross') ? 'is-invalid' : '' }}" type="number" name="gross" id="gross" value="{{ old('gross', $risk->gross) }}" step="1">
                @if($errors->has('gross'))
                    <div class="invalid-feedback">
                        {{ $errors->first('gross') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('action') ? 'is-invalid' : '' }}" name="action" id="action">
                    ...
                    @foreach(App\Models\Risk::ACTION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('action', $risk->action) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('action_details') ? 'is-invalid' : '' }}" name="action_details" id="action_details">{!! old('action_details', $risk->action_details) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('initiative') ? 'is-invalid' : '' }}" name="initiative_id" id="initiative_id">
                    @foreach($initiatives as $id => $initiative)
                        <option value="{{ $id }}" {{ (old('initiative_id') ? old('initiative_id') : $risk->initiative->id ?? '') == $id ? 'selected' : '' }}>{{ $initiative }}</option>
                    @endforeach
                </select>
