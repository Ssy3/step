    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.goals.update", [$goal->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $goal->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $goal->description) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('users') ? 'is-invalid' : '' }}" name="users[]" id="users" multiple>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (in_array($id, old('users', [])) || $goal->users->contains($id)) ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('users'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('strategic_plan') ? 'is-invalid' : '' }}" name="strategic_plan_id" id="strategic_plan_id">
                    @foreach($strategic_plans as $id => $strategic_plan)
                        <option value="{{ $id }}" {{ (old('strategic_plan_id') ? old('strategic_plan_id') : $goal->strategic_plan->id ?? '') == $id ? 'selected' : '' }}>{{ $strategic_plan }}</option>
                    @endforeach
                </select>
